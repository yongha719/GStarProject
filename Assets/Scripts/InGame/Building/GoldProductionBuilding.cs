...
public class GoldProductionBuilding : ProductionBuilding
{
    ...
    public virtual void ChangeCat(CatData catData, int index)
    {
        ...
        PlacedInBuildingCats[index].catNum = index;

        PlacedInBuildingCats[index].GoToWork(transform.position);
    }
    ...
    private void SetProductionTime()
    {
        ...
        {
            if (PlacedInBuildingCats[i] != null && (int)buildingType == (int)PlacedInBuildingCats[i].catData.GoldAbilityType)
            {
                // 능력이 건물의 종류와 같을 때
